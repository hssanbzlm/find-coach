...
<template>
  <v-form class="mt-5 form">
    <v-container>
      <v-row>
        <v-col cols="12">
          <v-text-field class="email-field" :model-value="coachDetails.email" disabled label="E-mail"></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12">
          <v-textarea v-model="state.message" @input="v$.message.$touch" :counter="100" label="Message" required
            ref="textArea" :disabled="isSentLoading"></v-textarea>
          <div class="input-errors" v-if="v$.message.$invalid && v$.message.$dirty">
            <span v-if="v$.message.required.$invalid" class="error-msg">The message is mandatory</span>
            <span v-if="v$.message.maxLength && !v$.message.required.$invalid" class="error-msg">{{
      ...
      <v-row no-gutters>
        <v-col>
          <BaseButton :isDisabled="v$.$invalid || isSentLoading" class="ma-2" color="#0277BD" textColor="text-white"
            @click="sendRequest">Send</BaseButton>
          <BaseButton class="ma-2" color="#757575" textColor="text-white" @click="cancelRequest">Cancel</BaseButton>
        </v-col>
      </v-row>
    </v-container>
  </v-form>
  ...
  <div v-if="isError || isSentError">
    <BaseAlert message="An error occured while sending your request" type="error" />
  </div>
</template>
